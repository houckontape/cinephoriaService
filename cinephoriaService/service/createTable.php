...
function getAnnotationForColumnType(ReflectionProperty $property): ?string
{
    ...
    if ($docComment) {
        ...
        if (str_contains($docComment, '@dbType TABLE')) {
            return 'table';
        }
...
function createTableFromClass(PDO $connection, string $className): void
{
    ...
    foreach ($properties as $property) {
        ...
        // on annote les propiété issus des tables avec le therme table et on ne creer pas la colonne
        // dans la table
        if($annotation==='table'){
            continue;
        }
    ...
    if (tableExists($connection, $tableName)) {
        ...
        try {
            ...
            return; //on quitte si la table n est aps correctement creer
        }
    }
    // appel pour inserer les données initiale
    if(method_exists($className,'getInitialData')){
        $initilalData = $className::getInitialData();
        if(is_array($initilalData)){
            insertInitialData($connection, $tableName, $initilalData);
        }
    }
}

/**
 * Insérer les premières données dans une table
 *
 * @param PDO    $connection
 * @param string $tableName
 * @param array  $data Tableau contenant les données à insérer dans la table
 */
function insertInitialData(PDO $connection, string $tableName, array $data): void
{
    if (empty($data)) {
        echo "Aucune donnée initiale à insérer dans la table `$tableName`.\n";
        return;
    }

    try {
        foreach ($data as $row) {
            // Générer une instruction INSERT dynamique
            $columns = implode(', ', array_keys($row));
            $placeholders = implode(', ', array_map(fn($value) => ':' . $value, array_keys($row)));
            ...
            echo "Données initiales insérées dans `$tableName`: " . json_encode($row) . "\n";
        }
    } catch (PDOException $e) {
        echo "Erreur lors de l'insertion des données initiales dans `$tableName`: " . $e->getMessage() . "\n";
    }
}
