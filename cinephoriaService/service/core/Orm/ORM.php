...
class ORM implements ORMInterface
{
    ...
    public function selectwithjoin(string $table, array $columns = ['*'], array $conditions = [])
    {
        $results = $this->select($table, $columns, $conditions);
        foreach ($results as &$result) {
            foreach ($result as $key => $value) {
                if(preg_match('/(?<tables>[A-Za-z]+)Id$/', $key,$matches)){
                    $relatedTable=$matches['tables'];
                    // recuperation des informations dans la sous table
                    $relatedData=$this->select($relatedTable,['*'],['id'=>$value]);
                    var_dump($relatedData);
                    if(!empty($relatedData)){
                        $result[$relatedTable]=$relatedData;
                        unset($result[$key]);
                    }
                }
            }
